...
- `--threshold K` — shorthand to set BOTH limits to the same CAFT cutoff (K). If unset, the lower and upper limits use their calibrated defaults (`1550` and `1250` K). Applies to single, `--input-list` batch, and HyperQueue modes alike.
- `--lfl-threshold K` / `--ufl-threshold K` — set the cutoff for the lower / upper limit independently (defaults `1550` / `1250` K; either also overrides `--threshold`). The LFL and UFL are read from separate CAFT contours, so calibrated cutoffs can differ. Both values are recorded per run in the JSON summary and the `lfl_threshold_K` / `ufl_threshold_K` columns of `SUMMARY.csv`.
- `--caft-workers N` — processes for the CAFT grid sweep, whose points are independent multiphase equilibria. Parallelising the grid across cores that would otherwise sit idle during the otherwise single-threaded sweep gives roughly a 3.5× speedup on 4 cores, with bit-for-bit identical results and no increase in peak memory (the per-molecule peak is set by the ML/SCF stage, not CAFT). Single-molecule mode auto-detects the task's CPU budget from `HQ_CPUS` (HyperQueue) or `OMP_NUM_THREADS`, falling back to the process affinity mask. **Under HyperQueue, pass it explicitly** (`--caft-workers $CPUS_PER_TASK`, as the [job_hq.sh.example](job_hq.sh.example) template does): HQ accounts CPUs per task but does **not** pin task affinity, so without `HQ_CPUS`/`OMP_NUM_THREADS` the affinity fallback would see the *whole node* and spawn far too many grid processes. `--input-list` batch mode defaults to `1`, since molecules already run in parallel across `--jobs`.
...
Template script: [job_hq.sh.example](job_hq.sh.example). Copy next to your data, edit `FLAMMAP_DIR` / `INPUT_DIR` / `OUTPUT_DIR` / `CPUS_PER_TASK` (and `LFL_THRESHOLD` / `UFL_THRESHOLD` if you want non-default cutoffs), then run it on the login node (it submits tasks to a HQ server you've already started in a separate SLURM worker job).
...
- CAFT flammability thresholds: default to `1550 K` (LFL) and `1250 K` (UFL), calibrated separately; override per run with `--lfl-threshold K` / `--ufl-threshold K` (each limit independently) or `--threshold K` (sets both). LFL and UFL are read from separate CAFT contours, so they can use different calibrated cutoffs.
