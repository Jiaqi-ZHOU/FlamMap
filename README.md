...
- `--caft-workers N` — processes for the CAFT grid sweep, whose points are independent multiphase equilibria. Single-molecule mode (including one-task-per-molecule HyperQueue) defaults to **all CPUs allocated to the task** (`--cpus-per-task`), parallelising the grid across cores that would otherwise sit idle during the otherwise single-threaded sweep — roughly a 3.5× speedup on 4 cores, with bit-for-bit identical results and no increase in peak memory (the per-molecule peak is set by the ML/SCF stage, not CAFT). `--input-list` batch mode defaults to `1`, since molecules already run in parallel across `--jobs`; override either with `--caft-workers N`.
